...
class PredectiveMaintenance():
    ...
    def create_pipeline(self):
        '''
        Predective Maintenace Data training and Validation
        '''
        ...
        X = final_df[['Type', 'Air temperature [K]',
                      'Process temperature [K]', 'Rotational speed [rpm]', 'Torque [Nm]',
                      'Tool wear [min]', 'Failure Type']]
        y = final_df[["Target"]]
        label = LabelEncoder()
        X.Type = label.fit_transform(X.Type)
        X['Failure Type'] = label.fit_transform(X['Failure Type'])
    ...
    def create_confusion_matrix(self, y_true, y_pred, name):
        ...
        try:
            ...
            c_plot = sn.heatmap(df_cm, fmt="d", linewidths=.2, linecolor='black', cmap="Oranges", annot=True,
                                annot_kws={"size": 16})
    ...
    def create_outputs(self, y_true, y_pred, X_test, name):
        ...
        '''
        pred_output = {"Actual Targe": y_true['target'].values, "Predicted Species": y_pred['Target'].values}
    ...
    def validate(self, y_true, y_pred, X_test):
        ...
        self.run.tag("PredectiveMaintenanceClassifierFinalRun")
...
if __name__ == "__main__":
    ...
    pred_classifier = PredectiveMaintenance(args)
    pred_classifier.create_pipeline()